...
describe('testGetChildNodes', () => {
    let testChild = (board_state, name) => {
        test(name, () => {
            ...
            expect([...result].map(val => ('0'.repeat(25) + val.toString(4)).slice(-25))).toEqual([...board_state].map((val, ind) => (val === '0' ? board_state.substring(0, ind) + '1' + board_state.substring(ind + 1) : '')).filter(val => val !== ''))
        })
    }
